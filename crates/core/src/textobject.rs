...
pub enum TextObjectKind { Inner, Around }
...
pub fn range_of(buf: &Buffer, pos: usize, obj: TextObject, kind: TextObjectKind) -> Option<std::ops::Range<usize>> {
    match obj {
...
fn pair_range(buf: &Buffer, pos: usize, open: char, close: char, kind: TextObjectKind) -> Option<std::ops::Range<usize>> {
    let len = buf.len_chars();
    if len == 0 { return None; }
    let rope = buf.rope();
    ...
    let mut open_at: Option<usize> = None;
    {
        ...
        loop {
            let c = rope.char(i);
            if c == close && i != pos { depth += 1; }
            else if c == open {
                if depth == 0 { open_at = Some(i); break; }
                depth -= 1;
            }
            if i == 0 { break; }
            i -= 1;
    ...
    let mut close_at: Option<usize> = None;
    {
        ...
        while i < len {
            ...
            if c == open { depth += 1; }
                ...
                if depth == 0 { close_at = Some(i); break; }
                depth -= 1;
            }
            i += 1;
        }
...
fn quote_range(buf: &Buffer, pos: usize, q: char, kind: TextObjectKind) -> Option<std::ops::Range<usize>> {
    let len = buf.len_chars();
    if len == 0 { return None; }
    let rope = buf.rope();
...
mod tests {
    ...
    fn inner_word() {
        ...
        // Cursor on "bar".
        assert_eq!(range_of(&b, 5, TextObject::Word, TextObjectKind::Inner), Some(4..7));
    }

    #[test]
    fn around_word_takes_trailing_space() {
        let b = Buffer::from_text("foo bar baz");
        assert_eq!(range_of(&b, 5, TextObject::Word, TextObjectKind::Around), Some(4..8));
    }
    ...
    fn inner_parens() {
        ...
        // Cursor inside "hello".
        assert_eq!(range_of(&b, 5, TextObject::Pair('(', ')'), TextObjectKind::Inner), Some(4..9));
        assert_eq!(range_of(&b, 5, TextObject::Pair('(', ')'), TextObjectKind::Around), Some(3..10));
    }
    ...
    fn nested_parens() {
        ...
        assert_eq!(range_of(&b, 4, TextObject::Pair('(', ')'), TextObjectKind::Inner), Some(4..5));
    }
    ...
    fn inner_double_quote() {
        let b = Buffer::from_text("foo \"hello\" bar");
        // Cursor inside "hello".
        assert_eq!(range_of(&b, 7, TextObject::Quote('"'), TextObjectKind::Inner), Some(5..10));
        assert_eq!(range_of(&b, 7, TextObject::Quote('"'), TextObjectKind::Around), Some(4..11));
    }
    ...
    fn quote_respects_escape() {
        ...
        assert_eq!(range_of(&b, 4, TextObject::Quote('"'), TextObjectKind::Inner), Some(3..7));
    }
    ...
    fn quote_line_scoped() {
        ...
        assert_eq!(range_of(&b, 2, TextObject::Quote('"'), TextObjectKind::Inner), None);
    }
}
